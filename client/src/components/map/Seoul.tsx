...
export default function Seoul({
	...
	const [vr, vrf] = useState([700, 600]); // view box 크기
	const [cmin, cminf] = useState([50, 50]); // view box min값
	const [dcoor, dcoorf] = useState([0, 0]); // 드래그시 위치 저장(이동거리 계산시 현재 위치반영)
	const [dpath, dpathf] = useState(0); // 이동한 거리 (단순 좌표 차이를 누적 계산함.)
	const [isDrag, isDragf] = useState(false); // 드래깅 여부
	const [cx, cy] = [
		window.innerWidth * 0.5 + (window.innerWidth * 0.5 > 600 ? 275.5 : 0.5 * (window.innerWidth * 0.5 - 55)),
		...
		<Frame
			onWheel={(e) => {
				...
				if (e.deltaY < 0) {
					let [x, y] = [vr[0], vr[1]];
					let [xx, yy] = cmin;
					const r = ((cx - clientX) ** 2 + (cy - clientY) ** 2) ** 0.5 / 300;
					if (x > 100) {
						x -= 40;
						if (clientX - cx < 0) {
							xx -= 10 * (1 + r) - 20;
			...
			onMouseMove={(e) => {
				...
				let [xmin, ymin] = cmin;
				if (isDrag) {
					dpathf(dpath + Math.abs(dcoor[0] - offsetX) + Math.abs(dcoor[1] - offsetY));
